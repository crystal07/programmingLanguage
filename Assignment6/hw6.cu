...
__global__ void maxpool(float *input, float *output, const int input_size, const int filter_size) {
    ...
    if (row * filter_size + filter_size - 1 >= input_size)
        return;
    if (col * filter_size + filter_size - 1 >= input_size)
        return;

    float max_val = 0;
    int ncol = col * filter_size;
    int nrow = row * filter_size;
    for (int i = nrow; i < nrow+filter_size; i++) {
        for (int j = ncol; j < ncol+filter_size; j++) {
            if (max_val < input[i * input_size + j])
                max_val = input[i * input_size + j];
        }
    }
    output[row * int(input_size/filter_size) + col] = max_val;
}
...
__global__ void gemm(float *a, float *b, float *c, const float alpha, const float beta, float *output, const int input_size){
    ...
    // CHANGE
    output[row * input_size + col] = result * alpha + beta * c[row * input_size + col];
}
