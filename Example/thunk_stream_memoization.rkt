...
; this is a silly addition function that purposely runs slowly 
(define (slow-add x y)
...
(define (my-mult2 x p) ;; assumes x is >= 0
  (cond [(= x 0) 0]
        [(= x 1) (my-force p)]
        [#t (+ (my-force p) (my-mult2 (- x 1) (my-force p)))]))
...
; first try on nats stream. 
; This one takes an argument and returns a thunk.
(define (nats0 x)
  (lambda () (cons x (nats0 (+ x 1)))))

; transform nats0 
(define (nats-aux x)
  (cons x (lambda () (nats-aux (+ x 1)))))

; then finally nats1 using nats-aux
(define nats1
  (letrec ([nats-aux (lambda (x) (cons x (lambda () nats-aux (+ x 1))))])
    (lambda () (nats-aux 1))))
